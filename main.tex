...
We model the upstream migration of coho salmon (\textit{Oncorhynchus kisutch}). As individuals move upriver, fish congregate in a holding area immediately downstream of the hatchery river dam. At discrete times, the hatchery staff opens the ladder gate at time $i$ to allow passage into holding tanks. Between successive gate-opening events ($\Delta i$), additional coho salmon arrive and join the backlog in the holding area, increasing the number of individuals awaiting passage. Let $X$ represent the true daily accumulation rate (also fish density at the river dam) in units of fish/day at time $i$, and E denote the number of days elapsed between consecutive gate openings ($ E_i =\Delta i; E_{i=1} = 0$). Prior to each gate opening (at time \textit{i}), the crew conducts a visual count $N_i$ of accumulated fish.
