data {
  ...
  int N_plate; // Total number of observations in qPCR standard samples for only detected samples
  array[N_st_q] int Z_qst; // Presence/Absence response of qPCR standard data
  array[N_st_qp] int plate_idx; // Plate index
...
transformed parameters {
  ...
  for (i in 1:N_st_qp) {
    mu_st[i] = eta_0[plate_idx[i]] + (eta_1 * S_q_p[i]);
