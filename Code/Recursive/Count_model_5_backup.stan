...
transformed parameters{
	...
	// vector[N_j_air] D_air;
	// vector[N_j_air] sigma;
	// vector[N_ft_air] epsilon;
	// 
	// eta = (log(mean_D_air) - D_wat) + error;
	// for (i in 1:N_j_air){
	// 	D_air[i] = log(mean_D_air[ft_idx[i]]) + epsilon[i];
	// }
	////////////////////////////////////////////////////////////////////// Water to Air
	// for (i in 1:N_j_air){
	// 	D_wat[air_wat_idx[i]] = log10((mean_D_air[ft_idx[i]]- epsilon[i]) / eta[air_filt_idx[i]]);
	// 	// epsilon[ft_idx[i]] = log10(mean_D_air[ft_idx[i]]) - D_wat[air_wat_idx[i]] + log(eta[air_filt_idx[i]]);
	// 	// sigma[i] = mean_D_air[ft_idx[i]] - (10^(D_air[i]));
	// }
	////////////////////////////////////////////////////////////////////// Bernuli Model
	//////////////////////////////////////////////////////////////////////// Standards
	for (i in 1:N_st_q){
	...
	//////////////////////////////////////////////////////////////////////// Air to Water
	mean_D_air = log(to_vector(average_by_idx(N_j_air, N_ft_air, to_array_1d((exp(D_air))), ft_idx)));
	for (i in 1:N_ft_air){
		// D_wat[air_wat_idx2[i]] = (mean_D_air[ft_idx2[i]] + epsilon[ft_idx2[i]]);
		D_wat[air_wat_idx2[i]] = log(exp(mean_D_air[ft_idx2[i]] + epsilon[ft_idx2[i]]) / exp(eta[air_filt_idx2[i]]));
	}
	//////////////////////////////////////////////////////////////////////// Water samples
	...
	for (i in 1:N_j_wat){
		...
		lambda[t1_idx[i]] = exp(D_wat[i]*beta_0);
	}
}
// 
model {
  ...
  //////////////////////////////////////////////////////////////////////// Count model
	...
	eta ~ normal(-2, 4);
	// for (i in 1:N_j_air){
	// 	D_air[i] ~ normal(mean_D_air[ft_idx[i]], exp(epsilon[i]));
	// }
	// Other
	// epsilon ~ normal(0, exp(tau));
	tau ~ normal(tau_p1,tau_p2);
	D_air ~ normal(0,3);
	// D_air ~ normal(D_air,sigma);
	// sigma ~ gamma(1,1);
}
