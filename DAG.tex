...
\node (air) [node, below=of Fish_TRUE, fill=myBlue]  at (8,0.20) {
    $\begin{array}{c}
        \ln(A_{ijb}) = \eta_{j} + \ln(W_{i}) + \varepsilon_{ij} + \delta_{ijb} \\
        \varepsilon_{ij} \sim \mathcal{N}(0,\tau_j) \\
        \delta_{ijb} \sim \mathcal{N}(0,\rho_j)
...
\node (visual) [node, below=of Fish_TRUE, fill=myBlue]  at (-7,0) {
    $\begin{array}{c}
        ...
        \lambda_{i} = X_i \cdot E_{i} \\
        N_{i} \sim Negative\ Binomial(\lambda_{i},20)
    \end{array}$
};
...
\node (qPCR_en_wat) [node, below=of water, fill=myBlue]  at (0,-3) {
    ...
    U_i = W_i / V_{itp} \\
        ...
        \psi_{i} = 1 - exp(-U_{i} \cdot \phi) \\
        Y_{itp} \sim \mathrm{Normal} (\mu_{ip}, \sigma_{i}) \quad \textnormal{if } Z_{it} = 1 \\
        \mu_{ip} = \beta0 + \beta1_{p} \cdot \ln (U_{i} / G) \\
        \sigma_{i} = \exp(\gamma0 + \gamma1 \cdot \ln (U_{i}))
    \end{array}$
};
...
\node (qPCR_en_air) [node, below=of air, fill=myBlue]  at (8,-3) {
    ...
    Q_{ijb} = A_{ijb} \cdot S_{ij} / V_{ijbtp} \\
        ...
        \psi_{ijb} = 1 - exp(-Q_{ijb} \cdot \phi) \\
        Y_{ijbtp} \sim \mathrm{Normal} (\mu_{ijb}, \sigma_{ijb}) \quad \textnormal{if } Z_{ijbt} = 1 \\
        \mu_{ijb} = \beta0 + \beta1_{p} \cdot \ln (Q_{ijb}) \\
        \sigma_{ijb} = \exp(\gamma0 + \gamma1 \cdot \ln (Q_{ijb}))
    \end{array}$
};
