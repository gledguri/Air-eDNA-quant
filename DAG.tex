...
%%%%%%%%%%% Air %%%%%%%%%%%
\node (air) [node, below=of Fish_TRUE, fill=myBlue]  at (8,0.35) {
    $\begin{array}{c}
        %\ln(W_{i}) = \eta_{j} + \ln(A_{ij}) + \varepsilon_{ij} \vspace{3px}\\
...
\node (visual) [node, below=of Fish_TRUE, fill=myBlue]  at (-7,0) {
    $\begin{array}{c}
        N_{i} \sim Poisson(\lambda_{i} \cdot E_{i})
    \end{array}$
};
...
\node[anchor=south west] at ([xshift=-15pt, yshift=-15pt] qPCR_st.south east) {\textbf{(6)}};
 ...
 % Arrows
        ...
        \draw [arrow] (qPCR_en_air) -- node[midway, left] {$A_{ijb}$} (air);
        \draw [arrow] (qPCR_en_wat) -- node[midway, left] {$W_i$} (water);

        \draw [arrow] (qPCR_st) -- node[midway, left, xshift=-8pt] {
        $\begin{array}{c}
            \phi0, \phi1 \\ \beta0, \beta1_p \\ \gamma0, \gamma1
        \end{array}$} (qPCR_en_wat);
        \draw [arrow] (qPCR_st) -- node[midway, right, xshift=8pt] {
        $\begin{array}{c}
            \phi0, \phi1 \\ \beta0, \beta1_p \\ \gamma0, \gamma1
        \end{array}$} (qPCR_en_air);
...
\small{
      \begin{tabular}{cl}
    &\textbf{Parameters} \\
    $\lambda$&fish density  \textit{(fish/day)}\\
    $\psi$&probability of positive qPCR amplification \\
    $\phi0, \phi1$&intercept and slope between $\psi$ and\\
